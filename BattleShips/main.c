...
void loadGame(char gameBoard[10][10], char originalBoard[10][10], Ship *ships, Stats *pstats) {
    ...
    if (ptr == NULL) {
        printf("\n[loadGame] Error! File could not open properly");
        return;
    }

    if (fread(originalBoard, sizeof(char[10][10]), 1, ptr) < 1) {
        printf("\n[loadGame] Failed to write game board.");
        return;
    }

    if (fread(gameBoard, sizeof(char[10][10]), 1, ptr) < 1) {
        printf("\n[loadGame] Failed to write game board.");
        return;
    }

    if (fread(ships, sizeof(Ship[5]), 1, ptr) < 1) {
        printf("\n[loadGame] Failed to write ships.");
        return;
    }

    if (fread(pstats, sizeof(Stats), 1, ptr) < 1) {
        printf("\n[loadGame] Failed to write ships.");
        return;
    }
...
bool getInput(char input[]) {
    printf("\nEnter coordinates to fire (Ex: A1) (QQ to quit): ");
    scanf("%s", input);
